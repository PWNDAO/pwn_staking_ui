...
export const calculateUserVotingMultiplier = (
  epochToCalculateIn: number,
  stakesWithVotingPower: Readonly<StakeDetail[]>,
): number => {
...
export const getFormattedVotingPower = (
...
export const getStakesDetails = async (
  stakeIds: bigint[] | readonly bigint[],
): Promise<Readonly<StakeDetail[]>> => {
...
export const getSecondsTillNextEpoch = (
  initialEpochTimestamp: number,
): number => {
  const currentTimestamp = Math.floor(Date.now() / 1000);
  // Calculate which epoch we're currently in
  const currentEpochNumber =
    Math.floor((currentTimestamp - initialEpochTimestamp) / SECONDS_IN_EPOCH) +
    1;
  // Calculate the timestamp of the next epoch
  const nextEpochTimestamp =
    initialEpochTimestamp + currentEpochNumber * SECONDS_IN_EPOCH;
  // Calculate time remaining until next epoch
  return nextEpochTimestamp - currentTimestamp;
};

export const getTimeTillNextEpochStringified = (
  initialEpochTimestamp: number,
): string => {
  return formatSeconds(getSecondsTillNextEpoch(initialEpochTimestamp));
};

export const getStartDateOfEpoch = (
  initialEpochTimestamp: number,
  epochNumber: number,
): Date => {
  epochNumber = epochNumber - 1; // epoch starts on 1
  return new Date(
    (initialEpochTimestamp + epochNumber * SECONDS_IN_EPOCH) * 1000,
  );
};

export const getStartDateOfEpochFormatted = (
  initialEpochTimestamp: number,
  epochNumber: number,
): string => {
