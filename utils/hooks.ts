...
export const useUserStakes = (
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Readonly<StakeDetail[]>> => {
      ...
      const sentStPwnNftsIds = sentStPwnNfts.map(
        (sentStPwnNft) => sentStPwnNft.args.tokenId,
      );
      ...
      return await getStakesDetails(
        ownedStPwnNfts.map((ownedStPwnNft) => ownedStPwnNft.args.tokenId!),
      );
    },
  });
};
...
export const useUserVotingPower = (
  walletAddress: Ref<Address | undefined>,
  currentEpoch: Ref<bigint | undefined>,
  chainId: Ref<SupportedChain>,
) => {
  return useQuery({
    ...
    enabled: computed(
      () =>
        walletAddress.value !== undefined && currentEpoch.value !== undefined,
    ),
    queryFn: async () => {
      return await readContract(wagmiAdapter.wagmiConfig, {
        address: VE_PWN_TOKEN[chainId.value],
...
export const useUserStakesWithVotingPower = (
  walletAddress: Ref<Address | undefined>,
  currentEpoch: Ref<number | undefined>,
  chainId: Ref<SupportedChain>,
) => {
  return useQuery({
    queryKey: [
      "userStakesWithVotingPower",
      walletAddress,
      currentEpoch,
      chainId,
    ],
    ...
    queryFn: async (): Promise<Readonly<StakeDetail[]>> => {
      const stakeIdsWhereBeneficiary = await readContract(
...
export const useUserCumulativeVotingPowerSummary = (
  walletAddress: Ref<Address | undefined>,
  epochs: Ref<bigint[] | undefined>,
  chainId: Ref<SupportedChain>,
) => {
  return useQuery({
    ...
    queryFn: async (): Promise<PowerInEpoch[]> => {
      ...
      const strippedParsedStakerPowers = parsedStakerPowers.filter(
        (stakerPower, index) => {
          // Keep the number if it's not zero
          if (stakerPower.power !== 0n) return true;

          // If it's zero, check if there's a non-zero number ahead
          return parsedStakerPowers
            .slice(index + 1)
            .some((_stakerPower) => _stakerPower.power !== 0n);
        },
      );

      const lastEpochWithPower =
        strippedParsedStakerPowers[strippedParsedStakerPowers.length - 1];
      // add 2 epochs with 0 power at the end results
      strippedParsedStakerPowers.push({
        epoch: lastEpochWithPower.epoch + 1n,
        power: 0n,
      });
      strippedParsedStakerPowers.push({
        epoch: lastEpochWithPower.epoch + 2n,
        power: 0n,
      });
...
export const useUserVestedTokens = (
  walletAddress: Ref<Address | undefined>,
  chainId: Ref<SupportedChain>,
) => {
  return useQuery({
    ...
    enabled: computed(
      () =>
        !!walletAddress.value &&
        PWN_VESTING_MANAGER?.[chainId.value as 1] !== undefined,
    ),
    queryFn: async (): Promise<VestingDetail[]> => {
      ...
      const createdVestings = await getLogs(client!, {
        ...
        event: parseAbiItem(
          "event VestingCreated(address indexed owner, uint256 indexed amount, uint256 indexed unlockEpoch, uint256 initialEpoch)",
        ),
        args: {
      ...
      const deletedVestings = await getLogs(client!, {
        address: PWN_VESTING_MANAGER[chainId.value as 1],
        event: parseAbiItem(
          "event VestingDeleted(address indexed owner, uint256 indexed amount, uint256 indexed unlockEpoch)",
        ),
        args: {
          owner: walletAddress.value!,
        },
      ...
      const noLongerActiveVestings = deletedVestings.map((deletedVesting) => {
        return {
          amount: deletedVesting.args.amount!,
          unlockEpoch: deletedVesting.args.unlockEpoch!,
        };
      });
      ...
      for (const createdVesting of createdVestings) {
        if (
          noLongerActiveVestings.every(
            (inactiveVesting) =>
              inactiveVesting.unlockEpoch !== createdVesting.args.unlockEpoch,
          )
        ) {
          activeVestings.push({
