...
export async function getSafeWalletThreshold(
  safeAddress: Address,
  chainId: number,
) {
...
export async function sendTransaction<
  ...
  >,
>(
  transaction: WriteContractVariables<
    TAbi,
    TFunctionName,
    TArgs,
    PwnWagmiConfig,
    PwnWagmiConfig["chains"][number]["id"]
  >,
  ...
  const [writeTxError, txHash] = await to(
    writeContract(wagmiAdapter.wagmiConfig, transaction),
  );
  ...
  if (step && (!threshold || threshold === 1)) {
    try {
      ...
      console.error("Error while getting a transaction: ", error);
    }
  }
    ...
    txReceipt = await new Promise<TransactionReceipt>((resolve) => {
      ...
      const unwatch = watchContractEvent(wagmiAdapter.wagmiConfig, {
        ...
        async onLogs(logs) {
          ...
          const log = logs.find(
            (_log) =>
              _log.args?.txHash === txHash || _log.transactionHash === txHash,
          );
          ...
          if (step && logs[0]?.transactionHash && threshold && threshold > 1) {
            ...
            const txReceipt = await getTransactionReceipt(
              ...
              { hash: logs[0].transactionHash, chainId: transaction.chainId },
            );
            console.log(
              "Contract wallet has successfully executed a transaction!",
            );
            resolve(txReceipt);
          }
          ...
          if (log) {
            ...
            const txReceipt = await getTransactionReceipt(
              wagmiAdapter.wagmiConfig,
              { hash: log.transactionHash, chainId: transaction.chainId },
            );
            console.log(
              "Contract wallet has successfully executed a transaction!",
            );
      ...
      console.log(
        `Waiting for ExecutionSuccess event on contract wallet on address=${contractWalletAddress} and chain ID=${transaction.chainId}.`,
      );
      ...
      getBlockNumber(wagmiAdapter.wagmiConfig, { chainId: transaction.chainId })
        .then((currentBlockNumber) => {
          return getPublicClient(wagmiAdapter.wagmiConfig, {
            chainId: transaction.chainId!,
          })!.getContractEvents({
            ...contractEventParameters,
        ...
        .then((logs) => {
          ...
          if (log) {
            console.log(
              "Found a corresponding event from a transaction execution in the past blocks.",
            );
            console.log(log);
            unwatch();
            if (step && !step.txHash) {
            ...
            return getTransactionReceipt(wagmiAdapter.wagmiConfig, {
        ...
        .catch((err) => {
          console.error(
            "Error while getting a past safe wallet ExecutionSuccess events.",
          );
          console.error(err);
        });
    });
