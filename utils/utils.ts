...
import { isAddress, isAddressEqual, erc20Abi } from "viem";
 ...
 */
export const parseRepeatedDecimal = (
  value: string,
): {
...
export const formatDecimalPoint = (
  amount: number | string,
  numbersBehindDecimalPoint = 4,
  showZeroes = true,
): string => {
  let stringAmount = "";
  if (typeof amount === "number" && isScientificNotation(amount)) {
    ...
    stringAmount =
      typeof amount === "number" ? amount.toString() : amount.replace(/,/g, "");
  }
  ...
  const decimalPlacesToKeep =
    numbersBehindDecimalPoint === 0
      ? 0
      : Math.max(numbersBehindDecimalPoint, firstNonZeroIndex + 1);

  const repeatedZeroes = parseRepeatedDecimal(
  ...
  if (!repeatedZeroes?.repeatedZeroes) {
    ...
    repeatedZeroes?.repeatedZeroes &&
    repeatedZeroes.repeatedZeroes > 0
  ) {
    formattedDecimal = decimalPart.slice(
      0,
      repeatedZeroes.repeatedZeroes + numbersBehindDecimalPoint,
    );
  }
  ...
  if (
    cleanedDecimal.length > numbersBehindDecimalPoint &&
    firstNonZeroIndex !== -1 &&
    firstNonZeroIndex < numbersBehindDecimalPoint
  ) {
  ...
  if (cleanedDecimal || showZeroes) {
    result +=
      "." + (showZeroes ? cleanedDecimal.padEnd(2, "0") : cleanedDecimal);
  }
