<template>
  <slot name="trigger" />
  <teleport to="body">
    <Transition type="transition" name="modal-show">
      <aside
        v-if="isOpenLocal"
            ...
            <header>
              <div class="modal__heading-container">
                <h2
                  :class="[
                    'modal__heading',
                    `modal__heading--align-${props.headingAlign}`,
                  ]"
...
const emit = defineEmits<{ (e: "update:isOpen", isOpen: boolean): void }>();
...
const isClickedOutside = (event: Event): boolean =>
    ...
    &__content {
      &--size {
        &-medium {
          ...
          max-width: calc(
            var(--app-max-width) * var(--modal-size-medium-mobile)
          );
        }
        ...
        &-large {
          ...
          max-width: calc(
            var(--app-max-width) * var(--modal-size-large-mobile)
          );
        }
      }
    }
