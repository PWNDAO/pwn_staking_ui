  ...
  <table class="table-positions">
    ...
    <tbody>
      <tr
        ...
        :class="[
          'table-positions__tr',
          { 'table-positions__tr--disabled': stake.isNextEpoch },
        ]"
      >
        ...
        <td class="table-positions__td">
          <BaseTooltip is-interactive :tooltip-text="stake.owner">
            <template #trigger>
              <span>
...
const nextEpoch = computed(() =>
  currentEpoch?.value ? currentEpoch?.value + 1 : undefined,
);
...
const userStakesWithVotingPowerQueryNextEpoch = useUserStakesWithVotingPower(
  address,
  nextEpoch,
  chainId,
);
...
const userStakesWithVotingPowerNextEpoch = computed(
  () => userStakesWithVotingPowerQueryNextEpoch.data.value,
);
...
const userStakesWithVotingPowerNextEpochFiltered = computed(() =>
  userStakesWithVotingPowerNextEpoch.value?.filter((stake) => {
    return (
      address.value !== stake.owner &&
      !userStakesWithVotingPowerFiltered.value?.find(
        (s) => s.stakeId === stake.stakeId,
      )
    );
  }),
);
