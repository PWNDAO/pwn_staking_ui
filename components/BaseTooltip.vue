...
const isCustomTooltipPlacement = (
  placement: TooltipPlacement,
): placement is CustomTooltipPlacement => {
  return CUSTOM_PLACEMENTS.some(
    (customPlacement) => customPlacement === placement,
  );
};
...
const handleTooltipShow = (): false | undefined => {
  if (!props.hasTooltip) {
  ...
  Z_INDEX:
    Number(
      window
        ...
        .getPropertyValue("--z-index-tooltip"),
    ) || 300, // fallback in case of issues
} as const;

const tippyProps = computed<TippyComponent["$props"]>(() => {
  return {
    animation: "shift-away",
    duration: props.placement.startsWith("center")
      ? undefined
