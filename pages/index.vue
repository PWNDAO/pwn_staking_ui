<template>
  <div class="homepage">
    <div v-if="showTestingTopBar" class="homepage__epoch-input">
      <template v-if="showEpochSwitcher">
        <div>Real epoch in contract: {{ currentEpoch }}</div>
        <div>
          Manually set epoch:
          <input
            v-if="epoch !== undefined"
            type="number"
            :value="epoch"
            @input="
              (event) => setEpoch((event.target as HTMLInputElement).value)
            "
          />
        </div>
      </template>
      ...
      <div class="homepage__header-summary-wrapper">
        ...
        <div class="homepage__summary-container">
          <div class="homepage__summary-box">
            <div class="homepage__box-subtitle">Idle Tokens</div>
            <BaseSkeletor v-if="isFetchingPwnTokenBalance" height="2" />
            <div v-else class="homepage__box-value">
              {{ pwnTokenBalanceFormatted }}
            </div>
          </div>

          <div v-if="vestedTokensAmount" class="homepage__summary-box">
            <div class="homepage__box-subtitle">Vested Tokens</div>
          ...
          <div class="homepage__summary-box">
            <div class="homepage__box-subtitle">Staked Tokens</div>
            <BaseSkeletor v-if="isFetchingUserStakes" height="2" />
            <div v-else class="homepage__box-value">
              {{ stakedTokensFormatted }}
            </div>
          </div>

          <div class="homepage__summary-box">
            <div class="homepage__box-subtitle">Current Voting Power</div>
          ...
          <div
            v-if="votingMultiplier !== undefined"
            class="homepage__summary-box"
          >
            <div class="homepage__box-subtitle">Voting Multiplier</div>
            <BaseSkeletor
              v-if="isFetchingUserStakesWithVotingPower"
              height="2"
            />
            <div v-else class="homepage__box-value">
              {{ votingMultiplierFormatted }}
            </div>
          </div>

          <div v-if="hasAnyStake" class="homepage__summary-box">
            <div class="homepage__box-subtitle">Time till next unlock</div>
            <div class="homepage__box-value">
              {{ nextUnlockFormatted }}
            </div>
          </div>
        </div>
    ...
    <div v-if="hasAnyStake" class="homepage__positions">
      <h3 class="homepage__positions-heading">
        Owned Positions ({{ stakesCount }})
      </h3>
      <TableOwnedPositions />
    </div>
    <div
      v-if="isFetchingUserStakes || isFetchingVestings"
      class="homepage__table-positions-loader"
    >
      <BaseSkeletor height="2" />
    </div>
    <div
      v-if="hasAnyVotingDelegation || hasAnyVotingDelegationNextEpoch"
      class="homepage__positions"
    >
      <h3 class="homepage__positions-heading">
        Voting Power Only Positions ({{ votingDelegationCount }})
      </h3>
...
const stakesCount = computed(
  () =>
    (stakesQuery.data.value?.length ?? 0) +
    (vestingsQuery.data.value?.length ?? 0),
);
...
const userStakesWithVotingPowerQuery = useUserStakesWithVotingPower(
  address,
  epoch,
  chainId,
);
const userStakesWithVotingPower = computed(
  () => userStakesWithVotingPowerQuery.data.value,
);

const userStakesWithVotingPowerFiltered = computed(() =>
  userStakesWithVotingPower.value?.filter((stake) => {
    return address.value !== stake.owner;
  }),
);
const userStakesWithVotingPowerNextEpochQuery = useUserStakesWithVotingPower(
  address,
  nextEpoch,
  chainId,
);
const userStakesWithVotingPowerNextEpoch = computed(
  () => userStakesWithVotingPowerNextEpochQuery.data.value,
);

// todo: do we need to filter out the current epoch ones here?
const userStakesWithVotingPowerNextEpochFiltered = computed(() =>
  userStakesWithVotingPowerNextEpoch.value?.filter((stake) => {
    return address.value !== stake.owner;
  }),
);

const votingDelegationCount = computed(
  () => userStakesWithVotingPowerFiltered.value?.length ?? 0,
);
const votingDelegationCountNextEpoch = computed(
  () => userStakesWithVotingPowerNextEpochFiltered.value?.length ?? 0,
);
const hasAnyVotingDelegation = computed(() => votingDelegationCount.value > 0);
const hasAnyVotingDelegationNextEpoch = computed(
  () => votingDelegationCountNextEpoch.value > 0,
);

const isFetchingUserStakesWithVotingPower = computed(
  () => userStakesWithVotingPowerQuery.isLoading.value,
);
...
const votingMultiplier = computed(() => {
  ...
  return calculateUserVotingMultiplier(
    epoch.value,
    userStakesWithVotingPower.value,
  );
});
...
const nextUnlockAt = computed(() => {
  ...
  for (const stake of stakesQuery.data.value ?? []) {
    ...
    if (
      epochWhereUnlocked > Number(epoch.value) &&
      (lowestRemainingEpochsForUnlock === undefined ||
        stake.remainingEpochs < lowestRemainingEpochsForUnlock)
    ) {
  ...
  for (const vesting of vestingsQuery.data.value ?? []) {
    const remainingEpochs = vesting.unlockEpoch - Number(epoch.value);
    if (
      vesting.unlockEpoch > Number(epoch.value) &&
      (lowestRemainingEpochsForUnlock === undefined ||
        remainingEpochs < lowestRemainingEpochsForUnlock)
    ) {
  ...
  return (
    secondsTillNextEpoch.value +
    lowestRemainingEpochsForUnlock * SECONDS_IN_EPOCH
  );
});
...
const totalPwnTokens = computed(() => {
  return (
    (vestedTokensAmount.value ?? 0n) +
    (stakedTokens.value ?? 0n) +
    (pwnTokenBalance.value ?? 0n)
  );
});
...
const showEpochSwitcher =
  import.meta.env.VITE_PUBLIC_SHOW_EPOCH_SWITCHER === "true";
const showChainSwitcher =
  import.meta.env.VITE_PUBLIC_SHOW_ONLY_MAINNET === "false";
